 ...
 */
extern const uint32_t *p_iipm_enable_irq_tbl[TNUM_PRCID];
 ...
 */
extern const uint32_t *p_bitpat_cfgint[TNUM_PRCID];
...
{
    if ((p_bitpat_cfgint[get_my_prcidx()][INTNO_MASK(intno) >> 5] & (1 << (INTNO_MASK(intno) & 0x1f))) == 0) {
        return false;
    }
    return true;
}

 ...
 * タイマ割込みハンドラ登録のための定数
 */
#define INTNO_TIMER_PRC1 ((0x00010000 | RP2040_TIMER0_IRQn) + 16) /* 割込み番号 */
#define INHNO_TIMER_PRC1 ((0x00010000 | RP2040_TIMER0_IRQn) + 16) /* 割込みハンドラ番号 */
#define INTNO_TIMER_PRC2 ((0x00020000 | RP2040_TIMER1_IRQn) + 16) /* 割込み番号 */
#define INHNO_TIMER_PRC2 ((0x00020000 | RP2040_TIMER1_IRQn) + 16) /* 割込みハンドラ番号 */
...
Inline void target_hrt_set_event(ID prcid, HRTCNT hrtcnt)
{
    ...
    if (prcid == PRC1) {
        sil_wrw_mem(RP2040_TIMER_ALARM0, current + hrtcnt);
    } else {
        sil_wrw_mem(RP2040_TIMER_ALARM1, current + hrtcnt);
    }
    ...
    if (target_hrt_get_current() - current >= hrtcnt) {
        if (prcid == PRC1) {
            sil_wrw_mem(RP2040_TIMER_ARMED, RP2040_TIMER_INT_ALARM_0);
        } else {
            sil_wrw_mem(RP2040_TIMER_ARMED, RP2040_TIMER_INT_ALARM_1);
        }

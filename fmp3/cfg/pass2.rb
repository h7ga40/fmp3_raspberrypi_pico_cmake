...
module Cfg1Out
  ...
  #
  def self.Read
    $romSymbolFileName = CFG1_OUT_SYMS if $romSymbolFileName.nil?
    # cfg1_out.symsの読み込み
    @symbolAddress = ReadSymbolFile($romSymbolFileName)

    # cfg1_out.srecまたはcfg1_out.dumpの読み込み
    begin
      if !$romImageFileName.nil?
        if $romImageFileName =~ /\.srec$/
          @cfg1SRec = SRecord.new($romImageFileName, :srec)
        else
          @cfg1SRec = SRecord.new($romImageFileName, :dump)
        end
      else
